    ...
    return (
        <>
           <div className="relative w-70">
                <input {...getInputProps(name)} name={name as string} type={isPassword ? (show ? "text" : "password") : "text"} className={`w-full h-10 rounded text-gray-400 border items-center justify-start px-3 mb-1 text-sm pr-9 ${errors?.[name]&&touched?.[name] ? "border-red-500 bg-red-200" : "border-gray-300"}`} placeholder={context}/>
                {isPassword && (
                    <button
                        type="button"
                        onClick={() => setShow((v) => !v)}
